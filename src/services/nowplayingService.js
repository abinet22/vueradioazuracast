...
axios.interceptors.response.use( x => {
    ...
    x => {
        console.error(`Execution time error for: ${x.config.url} - ${new Date().getTime() - x.config.meta.requestStartedAt} ms`)
        throw x;
    }
)
...
export default {
    ...
    getSongs(channel_id) {
        console.log("Nowplaying channel_id", channel_id);
    
        // Log the apiurl here
        let apiurl = config.api_url + '/nowplaying/' + channel_id;
        console.log("Nowplaying apiurl:", apiurl);
    ...
    _parseChannels( channels ) {
        let output = [];
            ...
            console.log("parchannels for loop ",channels)
